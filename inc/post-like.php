...
add_action( 'wp_enqueue_scripts', 'edumark_sl_enqueue_scripts' );
function edumark_sl_enqueue_scripts() {

	wp_enqueue_script( 'edumark-post-likes', EDUMARK_DIR_JS_URI . 'post-likes.js', array( 'edumark-ui-js' ), '0.5-s2', true );
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_edumark_process_simple_like', 'edumark_process_simple_like' );
add_action( 'wp_ajax_edumark_process_simple_like', 'edumark_process_simple_like' );
function edumark_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'edumark' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'edumark' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'edumark' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'edumark' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'edumark' );
	} else {
		$number = sl_format_count( $like_count ) . esc_html__( ' Like', 'edumark' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'edumark_show_user_likes' );
add_action( 'edit_user_profile', 'edumark_show_user_likes' );
function edumark_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'edumark' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'edumark' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
